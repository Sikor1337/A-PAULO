...
2. Samodzielnie wypełnia dynamiczną ankietę i ją wysyła. Może później edytować
   ten sam zapis; nie może utworzyć drugiej ankiety.
...
4. Backend zapisuje migawkę pytań i odpowiedzi. Późniejsze zmiany formularza nie
   zmieniają pytań zapisanych w istniejącej ankiecie.
5. Pracownicy konfigurują pytania i oglądają przesłane ankiety w zakładce
   `Rekrutacja → Odejścia`. Nie mogą wypełniać ankiety za wolontariusza. Admin
   ręcznie zmienia status wolontariusza na `Były`, gdy domyka jego odejście.
...
- `PUT /api/v1/recruitment/departures/me`
