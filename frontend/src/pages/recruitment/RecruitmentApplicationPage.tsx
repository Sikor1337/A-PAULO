...
const RecruitmentApplicationPage = () => {
  ...
  const { data: form, isLoading, isError, error } = useRecruitmentForm(invitationToken);
  const [answers, setAnswers] = useState<Record<string, unknown>>({});
  const submit = useMutation({
    mutationFn: (values: Record<string, unknown>) => recruitmentService.submit(invitationToken!, values),
  });
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    submit.mutate({
      ...answers,
      ...(form?.recipient_name && answers.full_name === undefined ? { full_name: form.recipient_name } : {}),
      ...(form?.recipient_email ? { email: form.recipient_email } : {}),
    });
  };
    ...
    <main className="min-h-dvh bg-[#1e2330] px-4 py-8 sm:py-12">
      <section className="mx-auto w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        <header className="bg-gradient-to-br from-indigo-600 to-violet-700 p-6 text-white sm:p-9">
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-indigo-100">{form?.description ?? 'Skorzystaj z indywidualnego linku otrzymanego od koordynatora.'}</p>
        </header>

        {!invitationToken ? (
          <div className="p-12 text-center text-rose-600">Ten formularz wymaga indywidualnego linku otrzymanego od koordynatora.</div>
        ) : isLoading ? (
          <div className="p-12 text-center text-gray-400">Ładowanie formularza…</div>
        ) : isError || !form ? (
          <div className="p-12 text-center text-rose-600">{parseApiError(error, 'Link jest nieprawidłowy, nieaktywny albo został już wykorzystany.')}</div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6 p-6 sm:p-9">
            ...
            {form.fields.map((field) => (
              <div key={field.id} className="text-sm font-bold text-gray-800">
                {field.field_type === 'radio' ? (
                  ...
                  <label htmlFor={`recruitment-${field.key}`}>
                ...
                <FieldControl
                  ...
                  disabled={field.key === 'email' && Boolean(form.recipient_email)}
                  value={
                    answers[field.key]
                    ?? (field.key === 'full_name' ? form.recipient_name : undefined)
                    ?? (field.key === 'email' ? form.recipient_email : undefined)
                  }
            ...
            {submit.isError && (
              <div className="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">
                {parseApiError(submit.error, 'Nie udało się wysłać formularza.')}
              </div>
            )}
            <div className="border-t pt-6">
              <button disabled={submit.isPending} className="min-h-12 w-full rounded-xl bg-emerald-600 px-6 font-bold text-white transition hover:bg-emerald-700 disabled:opacity-50">
                {submit.isPending ? 'Wysyłanie…' : 'Wyślij zgłoszenie'}
              </button>
              <p className="mt-3 text-center text-xs text-gray-400">Formularz można wysłać tylko raz, chyba że zgłoszenie zostanie zwrócone do poprawy.</p>
            </div>
          </form>
        )}
      </section>
